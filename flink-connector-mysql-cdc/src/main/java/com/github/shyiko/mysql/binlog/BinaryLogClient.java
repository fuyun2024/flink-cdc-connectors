...
public class BinaryLogClient implements BinaryLogClientMXBean {
    ...
    private final AtomicBoolean deserializeException = new AtomicBoolean(false);
    ...
    /**
     * @param gtidSet GTID set (can be an empty string). NOTE #1: Any value but null will switch
     *     BinaryLogClient into a GTID mode (this will also set binlogFilename to "" (provided it's
     *     null) forcing MySQL to send events starting from the oldest known binlog (keep in mind
     *     that connection will fail if gtid_purged is anything but empty (unless {@link
     *     #setGtidSetFallbackToPurged(boolean)} is set to true))). NOTE #2: GTID set is
    ...
    private void ensureEventDataDeserializer(
            EventType eventType,
