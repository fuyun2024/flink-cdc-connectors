...
public class ParallelReadEnumerator extends IncrementalSourceEnumerator {
    ...
    private void handleTableChangeCapture(List<TableChangeBean> tableChangeBeans) {
        ...
        for (TableChangeBean bean : tableChangeBeans) {
            ...
            if (bean.isAddedTable() && bean.isInitSync()) {
            ...
            } else if (bean.isAddedTable() && !bean.isInitSync()) {
                ...
                throw new FlinkRuntimeException("错误的同步类型 : " + bean.getOperation());
            }
    ...
    private void handleTableChangeAckEvent(TableChangeAckEvent ackEvent) {
        ...
        if (ackEvent.isAddedTable() && splitAssigner.isAddedTable(tableId)) {
            // 是新增表, 并且已经处理过的表, 不处理
            return;
        } else if (ackEvent.isDeletedTable() && !splitAssigner.isAddedTable(tableId)) {
    ...
    private boolean isProcessedTable(TableChangeBean bean) {
        TableId tableId = bean.getTableId();
        if (bean.isAddedTable() && splitAssigner.isAddedTable(tableId)) {
            // 是新增表, 并且已经处理过的表，才需要过滤
            return true;
        } else if (bean.isDeleteTable() && !splitAssigner.isAddedTable(tableId)) {
            // 是一张删除表，并且已经不再已完成的列表中，说明已经处理过了，需要过滤
            return true;
        } else {
            return false;
