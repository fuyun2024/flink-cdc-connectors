...
import com.ververica.cdc.connectors.sf.reader.ParallelSourceSplitReader;
import com.ververica.cdc.connectors.sf.reader.RestartStreamTaskSupplier;
...
public class IncrementalSource<T, C extends SourceConfig>
    ...
    public IncrementalSourceReader<T, C> createReader(SourceReaderContext readerContext)
        ...
        if (((JdbcSourceConfig) sourceConfig).isParallelReadEnabled()) {
            ...
            RestartStreamTaskSupplier restartStreamTaskSupplier = new RestartStreamTaskSupplier();
            Supplier<ParallelSourceSplitReader<C>> splitReaderSupplier =
                    () ->
                            new ParallelSourceSplitReader<>(
                                    readerContext.getIndexOfSubtask(),
                                    dataSourceDialect,
                                    sourceConfig,
                                    restartStreamTaskSupplier);
